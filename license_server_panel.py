...
#  ▸ Agora registra o total de usos de cada chave (campo "uses")
#  ▸ No painel, a coluna “HWID” foi substituída por “Usos”
...
def validate():
    ...
    data = _load()                         # (NEW) carregar todo o dicionário
    rec  = data.get(key)
    ...
    rec['uses'] = rec.get('uses', 0) + 1   # (NEW)
    _save(data)                            # (NEW) grava incremento
    ...
    if not rec['hwid']:
        rec['hwid'] = hwid
        _save(data)                        # (permanece gravando HWID)
...
def new():
    ...
    for _ in range(qty):
        k = rnd()
        data[k] = dict(expires=exp, hwid=None,
...
<div class=container py-4>
  ...
  <table class="table table-sm table-bordered align-middle">
    <thead class=table-light><tr>
      <th>Chave</th><th>Tipo</th><th>Expira</th><th>Usos</th><th>Status</th><th style=width:120px>Ações</th></tr></thead>
